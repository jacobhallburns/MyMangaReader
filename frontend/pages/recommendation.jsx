        ...
        <div onClick={onClose} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.85)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 10000, backdropFilter: 'blur(4px)' }}>
            <div onClick={(e) => e.stopPropagation()} style={{ width: '90%', maxWidth: '420px', background: 'var(--card-bg)', borderRadius: '24px', border: '1px solid var(--border-color)', padding: '2rem', boxShadow: '0 20px 40px rgba(0,0,0,0.4)' }}>
                ...
                <div style={{ margin: '1.2rem 0' }}>
                    ...
                    <textarea value={tempNotes} onChange={(e) => setTempNotes(e.target.value)} placeholder="What did you think?" style={{ width: '100%', padding: '1rem', borderRadius: '12px', background: 'var(--bg-color)', color: 'var(--text-main)', border: '1px solid var(--border-color)', minHeight: '100px', fontSize: '1rem', resize: 'none', boxSizing: 'border-box' }} />
                </div>
                {addError && (
        ...
        {manga.genres?.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem', marginBottom: '0.75rem' }}>
                {manga.genres.slice(0, 5).map(g => (
                    <span key={g} style={{ background: 'var(--border-color)', color: 'var(--text-muted)', padding: '0.15rem 0.55rem', borderRadius: '999px', fontSize: '0.68rem', fontWeight: '600', whiteSpace: 'nowrap' }}>{g}</span>
                ))}
            </div>
        )}
...
export default function Recommendations() {
    ...
    useEffect(() => {
        ...
        fetch('/api/manga/recommendations')
            .then(res => { if (!res.ok) { console.error('API FAILED:', res.status); return null; } return res.json(); })
            .then(data => { if (data) setRecs(data); setLoading(false); })
            .catch(err => { console.error('Fetch error:', err); setError('Failed to load recommendations.'); setLoading(false); });
    }, [isLoaded, isSignedIn]);

    // Initialize randomizer once data arrives
    useEffect(() => {
